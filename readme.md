...
+ Во-первых, позволит приложению зарускаться быстро, с минимальным количеством служб
+ Во-вторых, загружая службу и её зависимости асинхронно, сервер останется отзывчивым

Отзывчивость сервера также может дать выигрыш в производительности. Пока идёт создание отложенных служб, можно одновременно отправлять асинхронные запросы к уже запущенным службам доступа к БД, например, а к тому времени, когда службы запустятся, возможно, ответы от БД уже придут.

Если служба имеет своё дерево незапущенных зависимостей, то процесс создания экземпляра растянется на несколько циклов событий.

Когда происходит несколько конкурентных запросов на создание *singleton* службы, которая имеет незапущенные зависимости, велика вероятность создания дубликатов, что в данном случае неприемлемо.

Тогда контейнер должен начать исполнение кода создания экземпляра, которое растянется на несколько циклов событий, только для первого обратившегося, а остальным можно отправить один и тот же промис, который разрешится созданным экземпляром.
...
Далее, будет ожидать разрешения массива промисов на создание зависимостей *storage* и *logger*. Ожидать останется только экземпляр *storage*, так как *logger* уже разрешен.

В этом же цикле событий второй и третий запросы экземпляра *accum*, имея уже загруженные фабрики, рекурсивно запросят зависимость *storage* и получат один и тот же промис на его создание. Далее в коде идет непосредственный запрос экземпляра *storage*, в ответ придет тот же промис на его создание.

Ко второму циклу событий все конечные зависимости уже созданы. Каскадно разрешатся промисы первого цикла, будет создан экземпляр *storage* и разрешится промис на его создание.

В третьем цикле будет создан первый экземпляр *accum*, следом разрешатся промисы на создание двух других аккумуляторов, затем будут выполнена работа с ними.

Глубина зависимостей аккумуляторов на один уровень глубже, чем у *DerivedA*, поэтому свою работу аккумуляторы выполнят одним циклом позже, что и демонстрирует вывод программы.
